...
""".strip()
    ...
    # Inject the one-shot example if available
                ...
                if not voices:
                    ...
                    if labels_voice:
                        ...
                        if st.button("🎧 Generate audio for this newsletter"):
                            with st.spinner(
                                "Preparing a readable audio script and generating audio..."
                            ):
                                try:
                                    # Use OpenAI to turn the markdown newsletter into a smooth audio script
                                    client = build_openai_client()
                                    audio_script = (
                                        newsletter_markdown_to_audio_script(
                                            client=client,
                                            model=model,
                                            newsletter_markdown=newsletters[
                                                active_idx
                                            ],
                                        )
                                    )
